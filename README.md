...
🚥 | Property | Default | Description
--- | --- | --- | ---
🟡 | `arg: string[]\|'positional'` | `[]` | The argument switches to accept. A value of `[ '-o', '--output' ]` means `-o value`, `--output value`, `-o=value`, or `--output=value` would all be accepted. Use `positional` to accept all arguments that are not otherwise accepted.
...
🟢 | `type: ArgTypeName` | `listArg` if positional, otherwise `stringArg` | The type of the argument. The valid values are `booleanArg`, `'integerArg`,`stringArg`, and `listArg`. For the positional argument, only `listArg` is valid.
